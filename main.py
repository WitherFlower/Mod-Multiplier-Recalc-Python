    ...
    for mod in mods:
        ...
        if mod["acronym"] == "DA":
            ...
            if ("settings" in mod.keys() and
                ...
                new_mult *= (1 - 0.5 * abs(cs - mod["settings"]["circle_size"]))
            if ("settings" in mod.keys() and
                "approach_rate" in mod["settings"].keys()):
                new_mult *= (1 - 0.5 * abs(cs - mod["settings"]["approach_rate"]))
            if ("settings" in mod.keys() and
                "overall_difficulty" in mod["settings"].keys()):
                new_mult *= (1 - 0.5 * abs(cs - mod["settings"]["overall_difficulty"]))
            if ("settings" in mod.keys() and
                "drain_rate" in mod["settings"].keys()):
                new_mult *= (1 - 0.5 * abs(cs - mod["settings"]["drain_rate"]))
